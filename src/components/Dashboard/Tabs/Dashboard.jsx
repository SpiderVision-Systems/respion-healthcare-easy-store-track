...
// ── Download Modal ────────────────────────────────────────────────────────────
function DownloadModal({ records, onClose }) {
    ...
    const handleDownload = () => {
        ...
        const rows = inRange.map(r => ({
            "Installation Date": r.createdAt ? new Date(r.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }) : "—",
            ...
            "Referred By (Doctor)": r.refDoctor || "",
            "Installed By (Employee)": r.refEmployee || "",
            "Payment Type": r.paymentType || "",
            "Payment Mode": r.paymentMode || "",
            "Payment Account": r.paymentAcc || "",
            "Duration": parseDuration(r.duration),
            "Start Date": r.startDate ? new Date(r.startDate).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }) : "—",
            "Return Date": r.returnDate ? new Date(r.returnDate).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }) : "—",
            "Grand Total (₹)": r.grandTotal || 0,
            "Security Amount (₹)": r.securityAmount || 0,
            "Rent / Period (₹)": r.rentPerPeriod || 0,
            "Total Instalments": r.totalMonths,
            "Paid Instalments": r.paidMonths,
            "Collected (₹)": r.totalPaidAmt || 0,
            "Pending (₹)": r.pendingAmt || 0,
            "Status": r.status,
            "Returned": r.isReturned ? "Yes" : "No",
            "Accessories": (r.accessories || []).filter(a => a.name?.trim()).map(a => a.name).join(", "),
        }));
            ...
            for (let c = 0; c < colCount; c++) {
                ...
                if (ws[cellRef]) {
                    ws[cellRef].s = {
                        ...
                        fill: { fgColor: { rgb: r % 2 === 0 ? "F0FDF9" : "FFFFFF" }, patternType: "solid" },
        ...
        const label = from && to ? `${from}_to_${to}` : from ? `from_${from}` : to ? `upto_${to}` : "all";
        XLSX.utils.book_append_sheet(wb, ws, "Installations");
        XLSX.writeFile(wb, `installations-${label}.xlsx`);
    ...
    return (
        ...
        >
            <div
                className="bg-white rounded-3xl w-full sm:max-w-sm shadow-2xl overflow-hidden"
            ...
            >
                <div className="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-emerald-50 to-teal-50 border-b border-emerald-100">
                    ...
                    <div className="flex-1">
                        <h2 className="font-extrabold text-emerald-900 text-sm">Download Installations</h2>
                        <p className="text-[11px] text-emerald-600 font-medium">Filter by installation date range</p>
                    </div>
                    <button
                        onClick={onClose}
                        className="w-8 h-8 flex items-center justify-center rounded-full text-emerald-500 hover:text-emerald-800 hover:bg-emerald-100 transition-colors text-xl leading-none"
                    >
                        &times;
                    </button>
                </div>
                ...
                <div className="p-5 flex flex-col gap-4">
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 focus:outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-50"
                            />
                        </div>
                        <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {[
                            ...
                            <button
                                key={p.label}
                                onClick={p.fn}
                                className="text-[11px] font-semibold px-2.5 py-1 rounded-full border border-slate-200 bg-slate-50 text-slate-600 hover:border-teal-300 hover:bg-teal-50 hover:text-teal-700 transition-colors"
                            >
                                {p.label}
                            </button>
                        ))}
                    ...
                    <div className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm font-semibold ${inRange.length > 0 ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-slate-50 border-slate-200 text-slate-400"}`}>
                        <CheckCircle2 size={14} className={inRange.length > 0 ? "text-emerald-500" : "text-slate-300"} />
                        {inRange.length > 0
                            ? `${inRange.length} installation${inRange.length !== 1 ? "s" : ""} found`
                            : "No installations in this range"
                        }
                    </div>
                    ...
                    <div className="flex gap-2">
                        <button
                            onClick={onClose}
                            className="flex-1 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-600 text-sm font-bold hover:bg-slate-100 transition-colors"
                        >
                            Cancel
                        </button>
                        <button
                            onClick={handleDownload}
                            disabled={inRange.length === 0 || downloading}
                            className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                        >
                            <Download size={14} />
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="text-sm text-slate-600">
                            <span className="font-semibold">Payment:</span>{" "}
                            {r.paymentMode}
                            {r?.paymentAcc && <span className="ml-1 text-slate-500">({r.paymentAcc})</span>}
                        </div>
                        <div>Google Review Given? : {r.reviewGiven != null ? (r.reviewGiven ? "Yes" : "No") : "—"}</div>
                    </div>
                </div>
...
function TableRow({ r, index, onClick, onMarkReturned }) {
    return (
        <tr onClick={onClick} className={`hover:bg-slate-50 transition-colors cursor-pointer group border-b border-slate-50 last:border-0 ${r.allPaid ? "bg-emerald-50/20" : ""} ${r.isReturned ? "opacity-60" : ""}`}>
            ...
            <td className="px-4 py-3.5"><NextPaymentCell r={r} /></td>
            <td className="p-0">
                <div className="items-start gap-2 px-4 py-3.5">
                    <ReturnDateCell returnDate={r.returnDate} isReturned={r.isReturned} />
                </div>
            </td>
            <td className="px-4 py-3.5">
    ...
    const filtered = useMemo(() => {
        ...
        if (search.trim()) {
            ...
            list = list.filter(r =>
                ...
                r.refDoctor?.toLowerCase().includes(q) ||
                r.refEmployee?.toLowerCase().includes(q) ||
                r.phone?.includes(q) ||
                r.altPhone?.includes(q) ||
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                        ...
                        <div className="flex items-center gap-2 sm:ml-auto">
                            <button
                                ...
                                className={`relative p-2 rounded-xl transition-colors ${showAlerts ? "bg-amber-100 text-amber-700" : "hover:bg-slate-100 text-slate-500"}`}
                            >
                                <Bell size={18} />
                                {alertCount > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[9px] font-extrabold rounded-full flex items-center justify-center">{alertCount}</span>}
                            </button>
                            <button
                                ...
                                disabled={loading}
                                className="flex items-center gap-1.5 text-xs bg-amber-50 border border-amber-300 rounded-xl px-3 py-2 cursor-pointer hover:bg-amber-100 transition-colors disabled:opacity-50"
                            >
                                <RefreshCcw size={14} className={loading ? "animate-spin" : ""} />
                                <span className="hidden sm:inline">Refresh</span>
                            </button>
                            {/* ── Download button → opens date-range modal ── */}
                            <button
                                onClick={() => setShowDownloadModal(true)}
                                disabled={records.length === 0}
                                className="flex items-center gap-1.5 text-xs bg-emerald-50 border border-emerald-300 rounded-xl px-3 py-2 cursor-pointer hover:bg-emerald-100 transition-colors disabled:opacity-40"
                            >
                                <Download size={14} className="text-emerald-700" />
                                <span className="hidden sm:inline text-emerald-800 font-semibold">Download</span>
                            </button>
                    ...
                    <div className="flex flex-wrap gap-1.5 items-center">
                        {[
                            ...
                            <button
                                ...
                                className={`text-xs px-3 py-1.5 rounded-full font-semibold border transition-colors flex items-center gap-1 ${statusFilter === f.key ? "bg-teal-600 text-white border-teal-600" : "bg-white text-gray-600 border-gray-200 hover:border-teal-300"}`}
                            >
                                {f.label}
                                {f.key !== "all" && statusCounts[f.key] > 0 && (
                                    <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded-full ${statusFilter === f.key ? "bg-teal-500 text-white" : "bg-slate-100 text-slate-500"}`}>
                                        {statusCounts[f.key]}
                                    </span>
                                )}
                            </button>
                ...
                <div className="max-w-9xl mx-auto px-1 sm:px-2 lg:px-2 py-4 flex flex-col gap-4">
                    ...
                    {!loading && error && (
                        <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-start gap-3">
                            <AlertCircle size={17} className="text-red-500 shrink-0 mt-0.5" />
                            <div className="flex-1">
                                <div className="font-bold text-sm text-red-700">Failed to load data</div>
                    ...
                    {!loading && !error && (
                        <>
                            ...
                            {filtered.length === 0 && records.length > 0 && (
                                <div className="text-center py-16 text-slate-400 text-sm">No records match your search or filter.</div>
                            )}
