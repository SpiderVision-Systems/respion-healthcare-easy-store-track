...
// added not Day wise instead One for selected 
// "use client";
// import { useState, useEffect, useMemo } from "react";
// import { generatePatientPDF } from "@/lib/generatePatientPDF";
// import { useRouter } from "next/navigation";
...
// Added rounded amount and date selection
"use client";
import { useState, useEffect, useMemo } from "react";
import { generatePatientPDF } from "@/lib/generatePatientPDF";
import { useRouter } from "next/navigation";
...
export default function PatientForm({ onSuccess, user }) {
    ...
    // ── FIX: Track whether user has manually overridden the auto-dates ─────────
    ...
    // ── FIX: Safe numeric parsing — never coerces mid-type or empty strings to wrong values
    const rentPerPeriod = perPeriodRent === "" || perPeriodRent === "-"
        ? 0
        : Math.max(0, parseInt(perPeriodRent, 10) || 0);
    const security = securityAmount === "" || securityAmount === "-"
        ? 0
        : Math.max(0, parseInt(securityAmount, 10) || 0);

    // ── For daily mode: grandTotal = rentPerPeriod (single payment)
    // ── For month mode: grandTotal = rentPerPeriod * durationNum
    const rentTotal = isDailyMode ? rentPerPeriod : rentPerPeriod * durationNum;
    const grandTotal = rentTotal;

    // ── FIX: perInstalment uses Math.floor correctly; remainder goes to LAST slots
    // so the first (and most visible) instalment is always the clean base amount
    const perInstalment = isDailyMode ? grandTotal : (durationNum > 0 ? Math.floor(grandTotal / durationNum) : 0);
    const instalmentRem = isDailyMode ? 0 : (durationNum > 0 ? grandTotal % durationNum : 0);
    const hasCalc = rentPerPeriod > 0 && durationNum > 0;
    ...
    useEffect(() => {
        ...
        if (isDailyMode) {
            ...
            // ── Monthly schedule ───────────────────────────────────────────
            // FIX: Remainder +1 is added to the LAST `rem` slots, not the first.
            // This ensures the first instalment shown is always the clean base amount.
            const base = Math.floor(grandTotal / durationNum);
            const rem = grandTotal % durationNum;
            const [sy, sm, sd] = startDate.split("-").map(Number);
            setSchedule(Array.from({ length: durationNum }, (_, i) => {
                const { y, m, d } = addMonthsToYMD(sy, sm - 1, sd, i);
                // Give the +1 to the LAST `rem` slots so first instalments are clean
                const isRemSlot = rem > 0 && i >= durationNum - rem;
                return {
                    _id: `slot_${i}`,
                    month: getMonthLabel(y, m),
                    dueDate: ymdToISO({ y, m, d }),
                    amount: base + (isRemSlot ? 1 : 0),
                    status: "pending",
                    updatedAt: new Date().toISOString().slice(0, 10),
                };
            }));
    ...
    // ── Auto-set machine return date ONLY if user hasn't manually overridden it ─
    useEffect(() => {
        if (schedule.length > 0 && !returnDateManual) {
            setMachineReturnDate(schedule[schedule.length - 1].dueDate);
        }
    }, [schedule, returnDateManual]);
    ...
    function resetForm() {
        ...
        setPerPeriodRent(""); setSecurityAmount(""); setPaymentMode(""); setPaymentAcc(""); setDuration("");
        setStartDate(todayISO()); setMachineReturnDate("");
        // FIX: Reset manual override flags on form reset
        setStartDateManual(false);
        setReturnDateManual(false);
        setSchedule([]); setErrors({}); setSubmitted(false);
    }
                        ...
                        {step === 3 && (
                            <div className="flex flex-col gap-6">
                                <div>
                                    ...
                                    <DurationPicker value={duration} onChange={(val) => {
                                        setDuration(val);
                                        // FIX: Only auto-set start date if user hasn't manually changed it
                                        if (!startDateManual) {
                                            if (val.startsWith("d:")) setStartDate(todayISO());
                                            else setStartDate(sameDayNextMonth());
                                        }
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <TextInput
                                                    type="number"
                                                    placeholder={isDailyMode ? "e.g. 2000" : "e.g. 4000"}
                                                    min="0"
                                                    step="1"
                                                    value={perPeriodRent}
                                                    onChange={e => {
                                                        // FIX: Store raw string so the input stays responsive while typing.
                                                        // Numeric coercion happens only in the derived `rentPerPeriod` constant.
                                                        const raw = e.target.value;
                                                        // Strip leading zeros but preserve empty string for clearing
                                                        setPerPeriodRent(raw === "" ? "" : String(parseInt(raw, 10) || 0) === "0" && raw !== "0" ? "" : raw);
                                                    }}
                                                    error={errors.perPeriodRent}
                                                    className="pl-7"
                                                />
                                            </div>
                                            <ErrorMsg msg={errors.perPeriodRent} />
                                        ...
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <TextInput
                                                    ...
                                                    onChange={e => {
                                                        // FIX: Same safe raw-string approach for security amount
                                                        const raw = e.target.value;
                                                        setSecurityAmount(raw === "" ? "" : raw);
                                                    }}
                                            ...
                                            <div>
                                                ...
                                                {durationNum > 0 && grandTotal > 0 && (
                                                    <p className="text-[10px] text-teal-600 font-medium mt-0.5">
                                                        {isDailyMode
                                                            ? `1 rental · due on ${fmtDateShort(schedule[0]?.dueDate)}`
                                                            : `÷ ${durationNum} month${durationNum > 1 ? "s" : ""} = ${fmt(perInstalment)}/rental${instalmentRem > 0 ? ` (+₹1 last ${instalmentRem})` : ""}`
                                                        }
                                                    </p>
                                                )}
                                            </div>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            <FieldLabel required>
                                                ...
                                                {startDateManual && (
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            setStartDateManual(false);
                                                            setStartDate(isDailyMode ? todayISO() : sameDayNextMonth());
                                                        }}
                                                        className="ml-2 text-[10px] text-rose-400 hover:text-rose-600 font-bold normal-case tracking-normal underline"
                                                    >
                                                        reset to auto
                                                    </button>
                                                )}
                                            ...
                                            <TextInput
                                                ...
                                                value={startDate}
                                                onChange={e => {
                                                    // FIX: Mark as manually set so duration changes don't override it
                                        ...
                                        <div>
                                            <FieldLabel>
                                                Machine return date
                                                {!returnDateManual && (
                                                    <span className="ml-1 text-amber-500 font-bold normal-case tracking-normal text-[10px]">auto: last due date</span>
                                                )}
                                                {returnDateManual && (
                                                    <button
                                                        type="button"
                                            ...
                                            <TextInput
                                                type="date"
                                                value={machineReturnDate}
                                                onChange={e => {
                                                    // FIX: Mark as manually set so schedule rebuilds don't override it
                                                    setMachineReturnDate(e.target.value);
                                        ...
                                        <div className="flex items-start gap-3 bg-teal-50 border border-teal-200 rounded-xl px-4 py-3 mb-2">
                                            ...
                                            <p className="text-xs text-teal-700 leading-relaxed">
                                                {isDailyMode
                                                    ...
                                                    : <><span className="font-bold">{durationNum} rental{durationNum > 1 ? "s" : ""} · {fmt(perInstalment)}/rental · {fmt(grandTotal)} grand total</span>.{" "}
                                                        {`Due on the ${startDate.split("-")[2].replace(/^0/, "")}th of each month.`}
                                                        {instalmentRem > 0 && ` Last ${instalmentRem} rental${instalmentRem > 1 ? "s" : ""} are ₹1 more.`}
                                                    </>
                                                }
                                            </p>
                                        </div>
                            ...
                            <div className="flex flex-col gap-5">
                                ...
                                <div>
                                    ...
                                    <div className="bg-slate-50 border border-slate-100 rounded-xl px-4 divide-y divide-slate-100">
                                        ...
                                        <ReviewRow label={isDailyMode ? `Rent (due after ${durationNum} days)` : "Rent per month"} value={fmt(rentPerPeriod)} />
                                        <ReviewRow label="Grand Total" value={fmt(grandTotal)} />
                                        {!isDailyMode && <ReviewRow label="Per Rental" value={`${fmt(perInstalment)}${instalmentRem > 0 ? ` (last ${instalmentRem} are +₹1)` : ""}`} />}
                                        {security > 0 && <ReviewRow label="Security (refundable)" value={fmt(security)} />}
                                        <ReviewRow label="Payment Mode" value={paymentModeLabel} />
                                        <ReviewRow label="Start Date" value={fmtDateShort(startDate)} />
                                        <ReviewRow label={isDailyMode ? "Payment Due On" : "Rental Ends"} value={rentalEndDate ? fmtDateShort(rentalEndDate) : "—"} />
